...
export default function NotificationBell() {
  ...
  return (
    <div className="relative" ref={dropRef}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="relative w-9 h-9 flex items-center justify-center rounded-xl hover:bg-neutral-100 text-neutral-500 transition-colors"
      >
        <Bell size={19} />
        {unreadCount > 0 && (
          <span className="absolute top-1 right-1 min-w-[16px] h-4 px-1 flex items-center justify-center bg-danger text-white text-[10px] font-bold rounded-full ring-2 ring-white">
            ...
            {loading ? (
              ...
              notifications.map((n) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 hover:bg-neutral-50 transition-colors flex gap-2 ${!n.is_read ? 'bg-primary/5' : ''}`}
                >
