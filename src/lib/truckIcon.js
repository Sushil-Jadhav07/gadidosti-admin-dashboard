...
// (it would only look right pointing in one or two directions).
//
// This replaces the old approach of wrapping an external PNG file in a rotated SVG <image> —
// that PNG dependency is gone entirely now, which also permanently avoids the class of bug
// where a marker icon silently fails to load an external resource (see this app's earlier
// history: a wrong subdomain, then a relative path, then a same-origin path — a self-contained
// SVG with everything inlined has nothing left to fail to load).
//
// headingDeg comes from the Bolt GPS device's `course` field (a string in the vendor payload,
// e.g. "75" — Number.isFinite doesn't coerce strings, so it's coerced with Number() first or
// it'd always fall back to 0). Defaults to 0 (pointing up/north) when unknown.
