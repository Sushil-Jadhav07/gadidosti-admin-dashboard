      ...
      {loading ? (
        ...
        <div className="card overflow-hidden">
          <div className="px-5 pt-4 pb-4 flex flex-wrap items-center justify-between gap-3">
            <h3 className="text-base font-poppins font-semibold text-secondary">Disputes</h3>
            <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
              <div className="relative min-w-[170px]">
                <select value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value); setCurrentPage(1); }} className="form-select !py-2">
            ...
            <table className="data-table">
              ...
              <tbody>
                {paginated.map((dispute) => (
                  <tr key={dispute.id}>
                    <td className="font-medium text-neutral-800 whitespace-nowrap">{disputeRef(dispute)}</td>
                    <td className="whitespace-nowrap">{bookingRef(dispute)}</td>
                    ...
                    <td className="whitespace-nowrap">{new Date(dispute.date).toLocaleDateString('en-IN')}</td>
          ...
          <div className="flex items-center justify-between px-5 py-4 border-t border-neutral-100">
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => setCurrentPage((page) => Math.max(1, page - 1))} disabled={currentPage === 1} className="w-8 h-8 flex items-center justify-center rounded-lg border border-neutral-200 hover:bg-neutral-50 disabled:opacity-40 transition-colors"><ChevronLeft size={16} /></button>
              ...
              <button onClick={() => setCurrentPage((page) => Math.min(totalPages, page + 1))} disabled={currentPage === totalPages} className="w-8 h-8 flex items-center justify-center rounded-lg border border-neutral-200 hover:bg-neutral-50 disabled:opacity-40 transition-colors"><ChevronRight size={16} /></button>
            </div>
          </div>
        </div>
      )}
